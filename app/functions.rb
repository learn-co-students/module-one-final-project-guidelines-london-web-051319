...
def add_to_fav(user)
  user_liked = Favourite.all.find {|fav| fav.article_id == user.article_id && fav.user_id == user.user_id}
...
def favourite_articles(user_id)
  ...
  list.each do |el|
    ...
    article = Article.all.find {|article| article.id == article_id}
